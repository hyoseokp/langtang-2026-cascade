...
SCALES = [0.5, 0.75, 1.0, 1.5, 2.0, 3.0]; ICES = [0.1, 0.2, 0.3]; V0 = 35.4
...
def main() -> None:
    ...
    for ice in ICES:
        for sc in SCALES:
            ...
            vals = [rec[f"misfit_{g}"] for g in groups]; rec["misfit"] = float(np.mean(vals)) if np.all(np.isfinite(vals)) else np.nan; rows.append(rec)
    df = pd.DataFrame(rows); df.to_csv(ROOT / "figures" / "paper" / "table_vf_grid.csv", index=False)
    matplotlib.rcParams["font.family"] = "Arial"
    keys = [("misfit", "all evidence"), ("misfit_seismic", "seismic force"), ("misfit_heights", "gorge flow heights"), ("misfit_gauges", "flood gauges"), ("misfit_melt", "complete melting")]
    fig, axes = plt.subplots(2, 3, figsize=(15, 8.6), dpi=170); axes = axes.ravel()
    best = float(np.nanmin(df.misfit))
    for k, (ax, (key, title)) in enumerate(zip(axes, keys)):
        Z = df.pivot(index="ice", columns="volume_Mm3", values=key).values
        im = ax.imshow(Z, origin="lower", cmap="viridis_r", vmin=0, vmax=min(np.nanmax(Z) if np.isfinite(Z).any() else 1, 9), aspect="auto")
        ax.set_xticks(range(len(SCALES))); ax.set_xticklabels([f"{s*V0:.0f}" for s in SCALES], fontsize=12); ax.set_yticks(range(len(ICES))); ax.set_yticklabels([f"{i:g}" for i in ICES], fontsize=12)
        ax.set_xlabel("release volume (10$^6$ m$^3$)", fontsize=13); ax.set_ylabel("ice fraction", fontsize=13); ax.set_title(f"{'abcde'[k]}   {title}", fontsize=14, loc="left")
        for i in range(Z.shape[0]):
            for j in range(Z.shape[1]):
                if np.isfinite(Z[i, j]):
                    ax.text(j, i, f"{Z[i, j]:.1f}", ha="center", va="center", fontsize=11, color="w" if Z[i, j] > 0.6 * np.nanmax(Z) else "k")
                    if key == "misfit" and Z[i, j] <= best + 1.0:
                        ax.add_patch(plt.Rectangle((j - 0.5, i - 0.5), 1, 1, fill=False, edgecolor="red", lw=2.5))
        cb = fig.colorbar(im, ax=ax, shrink=0.85); cb.ax.tick_params(labelsize=11); cb.set_label("misfit (tolerance units)", fontsize=11)
    axes[-1].set_axis_off()
    axes[-1].text(0.0, 0.9, "misfit = mean over the group of" + chr(10) + "((model - target) / tolerance)$^2$, capped at 9" + chr(10) + chr(10) + "red boxes: within one unit of the best cell", fontsize=13, va="top", transform=axes[-1].transAxes)
